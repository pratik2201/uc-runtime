...
export class UserControl$Extended {
    ...
    private thisUc: Usercontrol;
    init(uc: Usercontrol) { this.thisUc = uc; }
    ...
    initializecomponent = (param0: IUcOptions): void => {
        ...
        if (param0.events.beforeInitlize != undefined) param0.events.beforeInitlize(this.thisUc);
        ucExt.isForm = (param0.parentUc == undefined);
        if (ucExt.isForm) {
            ucExt.dialogForm = this.thisUc;
        ...
        if (ucExt.isForm) {
            ucExt.PARENT = this.thisUc;
            ucExt.form = this.thisUc;
        ...
        ucExt.wrapperHT["#data"](ATTR_OF.BASE_OBJECT, this.thisUc);
        if (!ucExt.isForm) {
            ucExt.parentDependantIndex = pucExt.dependant.length;
            pucExt.dependant.push(this.thisUc);
        }
    ...
    show = ({ at = undefined, defaultFocusAt = undefined, decision = 'append' }:
        ...
        if (_extend.dialogForm == undefined)
            _extend.dialogForm = _extend.isForm ? this.thisUc : _extend.PARENT.ucExtends.dialogForm;
    ...
    showDialog = async ({ defaultFocusAt = undefined, at = undefined, keepCurrentVisible = true }: {
        ...
        if (ele) {
            ...
            await WinManager.push(this.thisUc);
        }
        ...
        if (_extends.dialogForm == undefined)
            _extends.dialogForm = this.thisUc;
    ...
    private hide = async () => {
        ...
        if (_ext.isDialogBox)
            await WinManager.pop(this.thisUc);
        Usercontrol.HiddenSpace.appendChild(_ext.wrapperHT);
        await _ext.Events.afterClose.fireAsync([this.thisUc]);

    }
    private destruct = async (): Promise<boolean> => {
        let main = this.thisUc;
...
export class Usercontrol {
    ...
    constructor() {
        this.ucExtends = new UserControl$Extended(this);
    }
